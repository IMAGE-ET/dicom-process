...
def sequence_name_to_folder(dir_to_clean='',dicom_suffix='dcm'):
    ...
    #allFiles = glob(cwd+'/*')
        for anyFile in files:
        	if not '.' in anyFile and anyFile != 'DICOMDIR':
        		os.rename(os.path.join(root, anyFile), os.path.join(root, anyFile) + ".dcm")
        	#elif anyFile.split('.')[-1] != "dcm":
        	#	os.rename(os.path.join(root, anyFile), os.path.join(root, anyFile) + ".dcm")
        	if anyFile.endswith(dicom_suffix):
        #for loose_dicom in glob(cwd+'/*'+dicom_suffix):

                    try:
                        dcm_img = dicom.read_file(os.path.join(root,anyFile),force=True)
                    except:
                        #print dcm
                        print("exception occured at {}".format(anyFile))
                        continue
                    ...
                    old_name = os.path.join(root,anyFile)
...
def anonymize_to_qbic(study_folder='',save_dir=SAVE_DIR):
    ...
    for sequence_dir in sequence_dirs:
        ...
        testFile = os.path.join(sequence_dir, files[0])
        for f in files:
            if f[0]=='.':
                break
            if not os.path.isfile(f):
                subdirs = glob(os.path.join(sequence_dir, testFile)+'/*/')
                for subdir in subdirs:
                    files = glob(subdir + '/**/*.dcm')
                    for f in files:
                        if f[0]=='.':
                            break
                        dicom_file = os.path.join(subdir, f)
